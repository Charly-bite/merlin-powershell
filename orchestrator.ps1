<#
.SYNOPSIS
    Dev Orchestrator - Development Server Manager (PowerShell Terminal)
...
function Get-AllListeningPorts {
    ...
    if ($IsLinux) {
        # Method: Linux ss (fast and modern)
        try {
            $raw = & ss -tlnp 2>&1
            foreach ($line in $raw) {
                # Example: LISTEN 0 128 0.0.0.0:8080 0.0.0.0:* users:(("node",pid=1234,fd=18))
                if ("$line" -match ':(\d+)\s+.*pid=(\d+)') {
                    $p = [int]$Matches[1]
                    $pid_ = [int]$Matches[2]
...
function Start-Project {
    ...
    try {
        if ($IsLinux) {
            ...
            # Use cmd.exe /c to launch any command reliably on Windows
...
function Stop-Project {
    ...
    try {
        ...
        $children = @()
        if ($IsLinux) {
            try { $children = & pgrep -P $targetPid 2>$null } catch {}
        } else {
            try {
    ...
    foreach ($proj in $Projects) {
        ...
        foreach ($pid_ in $pidsToKill) {
            ...
            try {
                ...
                $children = @()
                if ($IsLinux) {
                    try { $children = & pgrep -P $pid_ 2>$null } catch {}
                } else {
                    try {
                        $children = Get-CimInstance Win32_Process -ErrorAction SilentlyContinue |
...
function Write-Row {
    ...
    $memText = " $($S.Memory)"
    if ($memText.Length -gt $ColW.Mem) { $memText = $memText.Substring(0, $ColW.Mem) }
    Write-Host -NoNewline ($memText.PadRight($ColW.Mem)) @params -ForegroundColor $fgMeta
    Write-Host "|" -ForegroundColor DarkGray
}

function Show-Dashboard {
    param($Statuses, [int]$SelectedIndex = 0)
    ...
    # -- ASCII Art Header --
    Write-Host ""
    Write-Host '      ___  ____   ____  _   _ _____ ____ _____ ____      _  _____ ___  ____  ' -ForegroundColor DarkCyan
    Write-Host '     / _ \|  _ \ / ___|| | | | ____/ ___|_   _|  _ \    / \|_   _/ _ \|  _ \ ' -ForegroundColor DarkCyan
    Write-Host '    | | | | |_) | |    | |_| |  _| \___ \ | | | |_) |  / _ \ | || | | | |_) |' -ForegroundColor Cyan
    Write-Host '    | |_| |  _ <| |___ |  _  | |___ ___) || | |  _ <  / ___ \| || |_| |  _ < ' -ForegroundColor Cyan
    Write-Host '     \___/|_| \_\\____||_| |_|_____|____/ |_| |_| \_\/_/   \_\_| \___/|_| \_\' -ForegroundColor DarkCyan
    Write-Host ""
    Write-Host -NoNewline "    DEV SERVER MANAGER" -ForegroundColor White
    Write-Host -NoNewline "                                               " -ForegroundColor DarkGray
    Write-Host "v2.2" -ForegroundColor DarkGray
    Write-Host "    $("_" * 74)" -ForegroundColor DarkGray
    ...
    $colW = @{ Num=5; Name=18; Status=10; Addr=22; PID=8; Up=16; Mem=12 }
    $sep = "  +" + ("-" * $colW.Num) + "+" + ("-" * $colW.Name) + "+" + ("-" * $colW.Status) + "+" + ("-" * $colW.Addr) + "+" + ("-" * $colW.PID) + "+" + ("-" * $colW.Up) + "+" + ("-" * $colW.Mem) + "+"
    ...
    Write-Host -NoNewline "|" -ForegroundColor DarkGray
    Write-Host -NoNewline (" UPTIME".PadRight($colW.Up)) -ForegroundColor Cyan
    Write-Host -NoNewline "|" -ForegroundColor DarkGray
    Write-Host (" MEMORY".PadRight($colW.Mem)) -ForegroundColor Cyan -NoNewline
    Write-Host "|" -ForegroundColor DarkGray
    Write-Host $sep -ForegroundColor DarkGray
...
$Host.UI.RawUI.WindowTitle = "Dev Orchestrator - Server Manager"
Main
